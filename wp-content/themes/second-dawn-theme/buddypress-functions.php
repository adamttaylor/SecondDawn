...
class BP_Legacy extends BP_Theme_Compat {
	...
	public function sitewide_notices() {
		...
		if ( ! is_user_logged_in() )
			return;
		function bp_message_has_notices() {
		...
		$hideNotice = bp_message_has_notices()? '' : 'style="display:none"';
		echo '<div id="sitewide-notice" '.$hideNotice.' class="' . $class . '"><div class="inner">';
...
function bp_legacy_theme_ajax_messages_autocomplete_results() {
	...
	if ( ! empty( $user_ids ) ) {
		foreach ( $user_ids as $user_id ) {
			...
			echo '<span data-ajax="frump" id="link-' . esc_attr( $username ) . '" href="' . bp_core_get_user_domain( $user_id ) . '"></span>' . bp_core_fetch_avatar( array( 'item_id' => $user_id, 'type' => 'thumb', 'width' => 15, 'height' => 15, 'alt' => $ud->display_name ) ) . ' &nbsp;' . bp_core_get_user_displayname( $user_id ) . ' (' . esc_html( $username ) . ')' . "\n";
		}
	}
