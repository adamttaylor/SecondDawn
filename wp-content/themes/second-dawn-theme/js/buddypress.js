...
jq(document).ready( function() {
	...
	jq('#send_message_form #send-notice').change(function(){
		var att = jq(this).prop('checked')? 'disabled' : false;
		var op  = jq(this).prop('checked')? .5 : 1;
		jq('[name=send-to-input]').attr('disabled',att)	
		jq('[name=send-to-input]').css({'opacity':op})	
		
	})
	jQuery('.notice-item .thread-options').click(function(){
		var checkbx = jQuery(this).find('input[type=checkbox]');
		if(!jQuery(checkbx).prop("checked")){
			jQuery(this).parents('.notice-item').addClass('selected');
		}else{
			jQuery(this).parents('.notice-item').removeClass('selected');
		}
		jQuery(checkbx).prop("checked", !jQuery(checkbx).prop("checked"));
	});
	...
	$whats_new.focus( function(){
		$whats_new.stop().animate({height:150},300)
		jq('#whats-new-content').stop().animate({height:225},300,function(){
				jq('#whats-new-content').css({height:'auto'})
		});
	})
	$whats_new.blur( function(){
		if ( document.activeElement != this ) {
			if (!this.value.match(/\S+/)) {
				this.value = "";
				$whats_new.stop().animate({height:40},300)
				jq('#whats-new-content').stop().animate({height:40},300);
			}
		}
	})
	/**** Activity Posting ********************************************************/
	...
	jq("#aw-whats-new-submit").on( 'click', function() {
		...
		function(response) {
			...
			jq('#whats-new-content, #whats-new-form textarea').stop().animate({height:40},300);
	...
	jq( 'body.messages #item-body div.messages' ).on( 'change', '#message-type-select', function() {
		...
		var checkboxes = jq( "div input[type='checkbox']" ); 
		checkboxes.each( function(i) {
		...
		switch ( selection ) {
			case 'unread' :
				checkboxes = jq("#item-body .unread input[type='checkbox']");
				break;
			case 'read' :
				checkboxes = jq("#item-body .read input[type='checkbox']");
				break;
			case '' :
				checked_value = "";
				break;
		}
		jq(checkboxes).each(function(index,element){
			if(checked_value){
				jq(element).parents('.notice-item').addClass('selected');
			}else{
				jq(element).parents('.notice-item').removeClass('selected');
			}
			jq(element).prop('checked',checked_value)
		})
	...
	jq( 'body.messages #item-body div.messages' ).on( 'click', '.messages-options-nav a', function() {
		...
		checkboxes_tosend = '';
		checkboxes = jq("#message-threads div input[type='checkbox']");
