...
function setCurrentCharacter(num){
	...
	if(!num || $('.top-line #'+num+'.character').length==0){
...
$(document).ready(function(){
	...
	//Handlebars.registerPartial('backgroundItem',tmpstr.backgroundItem);
	//Handlebars.registerPartial('backgroundBox',tmpstr.backgroundBox);
	//Handlebars.registerPartial('equipmentItem',tmpstr.equipmentItem);
	//Handlebars.registerPartial('equipmentBox',tmpstr.equipmentBox);
	//Handlebars.registerPartial('skillBox',tmpstr.skillBox);
	//Handlebars.registerPartial('skillItem',tmpstr.skillItem);
...
function evaluateSkills(form){
		...
		var req_conditions = {'skills':['None'],'tags':[],'slots':{'low':0,'mid':0,'high':0}}
		var skills = $(form).find('.assign.skill-hold #sk_name').map(function(index,el){
			if(!$(el).parents('.skill-item').hasClass('wrong-class')){
				return $(el).val();
			}
		}).get();
		var skillset = []
		...
		req_conditions.slots.low  = parseInt($(form).find('#low-slots').val());
		req_conditions.slots.mid  = parseInt($(form).find('#mid-slots').val());
		req_conditions.slots.high = parseInt($(form).find('#high-slots').val());
		req_conditions.skills = req_conditions.skills.concat(skills);
		$(form).find('.assign.skill-hold .skill-item #tags').each(function(index, element) {
			if(!$(this).parents('.skill-item').hasClass('wrong-class')){
				if($(this).val().length>0){
					...
					req_conditions.tags = req_conditions.tags.concat(tagz)
				}
			}
        });
			...
			_.each(params.split('&&'),function(and){				
				...
				_.each(and.split('||'),function(or){
					...
					switch(type){
						...
						case 'slot':
						log('slot'+req_conditions.slots.high+' :?'+(req_conditions.slots.high > 0))
			...
			_.each(params.split('&&'),function(and){				
				...
				if(trueCnt<2){
					_.each(orz,function(or){
						...
						if(orArr[0] =='skill'){
							var mat = orArr[1];
							$(form).find('.assign.skill-hold .skill-item #sk_name[value="'+orArr[1]+'"]').parents('.skill-item').addClass('is_prereq')
						}
						if(orArr[0]=='tag'){
							var mat = orArr[1];
...
function init(){
	log('CURRENT CHARACTER: '+current_character)
	//cacheItems();
	pre_load_assets(function(){
		//cache.skills = _.groupBy(cache.skills,'sk_class');
		//set starting skills and buyskills
		...
		if(current_character==0){
			...
			if(hashArr.length==2){
				...
				var obj = staffFunction? $('.inv-menu .character.staffer') : $('.inv-menu #'+hashArr[1]+'.character');
				$('.inv-menu #'+hashArr[1]+'.character').addClass('active');
...
function initialFunction(){
	//1 Set the current character
	//determine if the current character can be selected, o	
}
